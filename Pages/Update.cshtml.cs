    ...
    public class UpdateModel : PageModel
    {
		...
		private DrinkingWaterModel GetById(int id)
		{
			...
			using (var connection = new SqliteConnection(_configuration.GetConnectionString("ConnectionString")))
			{
				...
				while (reader.Read())
				{
					...
					drinkingWaterRecord.Date =
						DateTime.Parse(reader.GetString(1), CultureInfo.CurrentUICulture.DateTimeFormat);
		...
		public IActionResult OnPost(int id)
		{
			using (var connection = new SqliteConnection(_configuration.GetConnectionString("ConnectionString")))
			{
				connection.Open();
				var tableCmd = connection.CreateCommand();
				tableCmd.CommandText = $"UPDATE drinking_water SET date =" +
				                       $" '{DrinkingWater.Date.ToString(CultureInfo.InvariantCulture)}'," +
				                       $" quantity = {DrinkingWater.Quantity} WHERE id = {id}";
